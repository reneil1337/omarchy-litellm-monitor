...
Item {
  ...
  function displayProvider(record) {
    ...
    return {
      ...
      // Long-range history (per-day tokens + request counts) and per-model
      // per-day tokens ride along from collectors that track much more than
      // the last 7 days. Panel falls back to recentDays when absent.
      history: Array.isArray(record.history) ? record.history : [],
      modelDaily: (record.modelDaily !== null && typeof record.modelDaily === "object" && !Array.isArray(record.modelDaily)) ? record.modelDaily : {},
